...
async function main() {
    ...
    console.log('starting workload: ', START_NOW, ' , sendTransactions: ', LIMIT_TRANSACTIONS);
    const FILE_NAME = 'log_worload_' + LIMIT_TRANSACTIONS + '_transactions_' + START_NOW;

    var objData = [];
    let count = 0;
    for (var i = 0; i < LIMIT_TRANSACTIONS; i++) {
        try {
            ++count;
            utilWorkload.sendPatientTransferExam(count, accountPatient, accountProfessional, IPFS_HASH, FILE_NAME, objData);
        } catch (e) {
            console.log('error for send transaction: ', e);
        }
    }
